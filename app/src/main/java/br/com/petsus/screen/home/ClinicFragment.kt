...
import br.com.petsus.screen.home.fragment.ClinicViewModel
...
class ClinicFragment : BaseFragment<FragmentClinicBinding>() {
    ...
    private fun load() {
        val id = binding?.containerClinic?.id ?: return
        childFragmentManager
            .findFragmentById(id)
            ?.cast<SupportMapFragment>()?.let { supportMapFragment ->
                            ...
                            providerClient.getCurrentLocation(
                                ...
                                viewModel.load(LatLng(currentLocation.latitude, currentLocation.longitude)).observe(viewLifecycleOwner) { clinics ->
                                    ...
                                    googleMap.animateCamera(
                                        CameraUpdateFactory.newLatLngZoom(LatLng(currentLocation.latitude, currentLocation.longitude), 15f)
                                    )
                                }
                            }
                        }
